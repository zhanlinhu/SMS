layui.use(['form','layer','laydate','table','laytpl'],function(){
    ...
    var tableIns = table.render({
        ...
        cols : [[
            ...
            {title: '操作', width:200, templet:'#noticeListBar',fixed:"right",align:"center"}
        ]]
    });
    ...
    function addNotice(edit){
        ...
        var index = layui.layer.open({
            ...
            type : 2,
            dataType:"json",
            content : "/SMS/notice/addPage.html",
            success : function(layero, index){
                ...
                if(edit){
                    ...
                    //body.find("#author").attr("readonly","readonly").addClass("layui-disabled")
                    body.find("#author").val(edit.author);
                    body.find("#notice_content").val(edit.content);
                    //body.find("#notice_content").setContent(edit.content);
                    form.render();
                } else {
    ...
    $(".delAll_btn").click(function(){
        ...
        if(data.length > 0) {
            ...
            layer.confirm('确定删除选中的公告？', {icon: 3, title: '提示信息'}, function (index) {
                $.get("/SMS/notice/deleteList.html",{
    ...
    table.on('tool(noticeList)', function(obj){
        ...
        if(layEvent === 'edit'){ //编辑
        	...
        	var index = layui.layer.open({
                title : "公告详情",
                type : 2,
                content : "/SMS/notice/look.html",
                success : function(layero, index){
                    var body = layui.layer.getChildFrame('body', index);
                    body.find("#title").html(data.title);
                    body.find("#author").html(data.author);
                    body.find("#newstime").html(data.newstime);
